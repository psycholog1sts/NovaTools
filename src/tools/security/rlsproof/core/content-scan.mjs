...
function scanCodeFile(filePath, text) {
  ...
  if (serviceRoleMatch) {
    findings.push(makeFinding({
      engine: 'native', rule: 'supabase-service-role-client', severity: 'critical',
      title: 'Supabase service-role credential identifier found in application code',
      path: filePath, line: lineNumberFor(text, serviceRoleMatch.index ?? 0),
  ...
  for (const match of text.matchAll(evalPattern)) {
    ...
    findings.push(makeFinding({
      engine: 'native', rule: 'dangerous-eval', severity: 'high', title: 'Dynamic code execution detected',
      path: filePath, line, evidence: rawLine, fingerprintSource: rawLine,
...
function scanSqlSecurityPatterns(filePath, text) {
  ...
  for (const match of text.matchAll(policyPattern)) {
    ...
    if (/\bauth\.role\s*\(\s*\)/i.test(statement)) {
      findings.push(makeFinding({
        engine: 'native', rule: 'supabase-policy-deprecated-auth-role', severity: 'medium',
        title: 'RLS policy uses deprecated auth.role()', path: filePath, line,
        evidence: 'RLS policy calls deprecated auth.role(); policy text intentionally omitted', fingerprintSource: statement,
    ...
    if (/auth\.jwt\s*\(\s*\)[\s\S]*?(?:user_metadata|raw_user_meta_data)/i.test(statement)) {
      findings.push(makeFinding({
        engine: 'native', rule: 'supabase-policy-user-metadata-authorization', severity: 'high',
        title: 'RLS policy may trust user-editable metadata for authorization', path: filePath, line,
        evidence: 'RLS policy references user-editable JWT metadata; policy text intentionally omitted', fingerprintSource: statement,
  ...
  for (let index = 0; index < functionHeaders.length; index += 1) {
    ...
    findings.push(makeFinding({
      engine: 'native', rule: 'supabase-public-security-definer', severity: 'high',
      title: `SECURITY DEFINER function is created in exposed public schema: ${functionName}`,
      path: filePath, line: lineNumberFor(text, start), evidence: `public.${functionName} is declared SECURITY DEFINER`,
  ...
  for (const match of text.matchAll(publicViewPattern)) {
    ...
    findings.push(makeFinding({
      engine: 'native', rule: 'supabase-public-view-without-security-invoker', severity: 'high',
      title: `Public view may bypass underlying RLS policies: ${viewName}`, path: filePath,
      line: lineNumberFor(text, match.index ?? 0),
      evidence: `create view public.${viewName} detected without security_invoker = true`, fingerprintSource: statement,
...
function collectSqlState(sqlFiles) {
  ...
  for (const { path: filePath, text } of sqlFiles) {
    ...
    for (const match of text.matchAll(createPattern)) {
      ...
      if (!created.has(table)) created.set(table, { path: filePath, line: lineNumberFor(text, match.index ?? 0) });
    }
    const rlsPattern = /alter\s+table\s+(?:if\s+exists\s+)?"?public"?\."?([a-zA-Z0-9_]+)"?\s+enable\s+row\s+level\s+security/gi;
    for (const match of text.matchAll(rlsPattern)) rlsEnabled.add(normalizeTableName(match[1]));
  }
...
export function scanVirtualFiles(files) {
  ...
  for (const file of files) {
    ...
    if (/^\.env(?:\..+)?$/i.test(name)) {
      findings.push(makeFinding({
        engine: 'native', rule: 'sensitive-env-file', severity: 'high',
        title: 'Secret-bearing environment file is present in the scanned tree', path: filePath, line: null,
        evidence: `${filePath} exists; file contents were not included in evidence`, fingerprintSource: filePath,
  ...
  for (const [table, location] of created) {
    ...
    findings.push(makeFinding({
      engine: 'native', rule: 'supabase-public-table-without-rls', severity: 'high',
