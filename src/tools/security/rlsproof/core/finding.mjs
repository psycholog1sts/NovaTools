...
export function makeFinding(input) {
  ...
  const identity = [String(input.engine), String(input.rule), input.path ?? '', input.line ?? '', String(input.title)].join('\u0000');
  const id = `gdn_${sha256Hex(identity).slice(0, 16)}`;
  const evidence = redactEvidence(input.evidence);
  const fingerprintSource = redactEvidence(input.fingerprintSource ?? evidence ?? input.title) ?? String(input.title);
  const fingerprintIdentity = [String(input.engine), String(input.rule), normalizeFingerprintPath(input.path), String(fingerprintSource)].join('\u0000');
