  ...
  container.innerHTML = featured.map(cat => `
    <a href="/categories/${cat.slug}" class="category-card">
      ...
      <div class="category-meta">
        <span class="category-count">${cat.toolCount || '12+'} tools</span>
        <span>→</span>
      </div>
    </a>
  `).join('');
    ...
    <div class="tool-card">
      <div class="tool-header">
        ...
        <div class="tool-badges">
          ${tool.popular ? '<span class="tool-badge tool-badge-popular">Popular</span>' : ''}
        </div>
      </div>
  ...
  container.innerHTML = newTools.map(tool => `
    <div class="tool-card">
      <div class="tool-header">
        ...
        <div class="tool-badges">
          ${tool.new ? '<span class="tool-badge tool-badge-new">New</span>' : ''}
        </div>
      </div>
      <h3 class="tool-name">${tool.name}</h3>
      <p class="tool-description">${tool.description}</p>
...
function initSearch() {
  ...
  // Demo search - in production this would query a real index
  const allTools = [
    ...
    { name: 'Image Compressor', slug: 'image/compress', category: 'Image Tools' },
    { name: 'Image Converter', slug: 'image/convert', category: 'Image Tools' },
    { name: 'Mortgage Calculator', slug: 'finance/mortgage-refinance', category: 'Finance Tools' },
    { name: 'Tax Estimator', slug: 'finance/tax', category: 'Finance Tools' },
    { name: 'JSON Formatter', slug: 'dev/json-formatter', category: 'Developer Tools' },
    { name: 'Regex Tester', slug: 'dev/regex-tester', category: 'Developer Tools' },
    { name: 'Word Counter', slug: 'text/word-counter', category: 'Text & Writing' }
  ];
  ...
  searchInput.addEventListener('input', (e) => {
    ...
    `).join('') || '<div class="search-no-results">No tools found</div>';
  });
}
