...
const defaultDeny = proveTableIsolation(state(base), {
  table: 'public.docs', operation: 'select', role: 'authenticated', identity: 'auth.uid()', column: 'owner_id',
});
assert.equal(defaultDeny.status, 'PROVEN');
assert.equal(defaultDeny.reason, 'default-deny-no-permissive-policy');
...
assert.equal(wrongRole.status, 'PROVEN');
assert.equal(wrongRole.reason, 'default-deny-no-permissive-policy');
