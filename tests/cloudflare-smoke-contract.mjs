...
assert.match(workflow, /id: reconcile_hosts[\s\S]*continue-on-error: true/);
assert.match(workflow, /if: always\(\)[\s\S]*node scripts\/resolve-cloudflare-pages-deployment\.mjs/);
assert.match(workflow, /if: always\(\)[\s\S]*node scripts\/smoke-cloudflare-pages\.mjs/);
assert.match(workflow, /id: apex_edge[\s\S]*continue-on-error: true/);
assert.match(workflow, /id: www_edge[\s\S]*continue-on-error: true/);
assert.match(workflow, /node scripts\/report-cloudflare-dns-evidence\.mjs/);
assert.match(workflow, /node scripts\/enforce-production-diagnostics-gate\.mjs/);
...
assert.match(hostReconciler, /canonical_dns/);
assert.match(hostReconciler, /CAA/);
